...
Per evitare che il server si blocchi durante la gestione di una singola richiesta, è stato utilizzato il modulo `threading`. Il thread principale rimane costantemente in ascolto (`accept()`), mentre per ogni client viene creato un thread dedicato alla funzione `handle_client`.
